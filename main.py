...
# Read One Blog 
@app.get("/blog", response_model=schemas.BlogResponse)
def get_blog(id:str, db: Session =Depends(get_db)):
...
def update_blog(id:str,blog:schemas.BlogCreate, db: Session =Depends(get_db), user = Depends(verify_token)):
...
# Delete Blog 
@app.delete("/blogs/{id}")
def delete_blog(id:str, db: Session =Depends(get_db), user = Depends(verify_token)):
